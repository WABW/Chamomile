...
package io.wabw.misc.mail.pop3;
...
import io.wabw.misc.mail.util.LineInputStream;
...
public class POP3Folder extends Folder {
    ...
    private volatile io.wabw.misc.mail.pop3.Protocol port;
    private int total;
    private int size;
    ...
    private volatile io.wabw.misc.mail.pop3.TempFile fileCache = null;
	...
	return new io.wabw.misc.mail.pop3.DefaultFolder(store);
    }
	...
	try {
	    ...
	    io.wabw.misc.mail.pop3.Status s = port.stat();
	    total = s.total;
	    size = s.size;
	    this.mode = mode;
	    if (store.useFileCache) {
		try {
		    fileCache = new io.wabw.misc.mail.pop3.TempFile(store.fileCacheDir);
	...
	if ((m = (io.wabw.misc.mail.pop3.POP3Message)message_cache.elementAt(msgno-1)) == null) {
    ...
    protected io.wabw.misc.mail.pop3.POP3Message createMessage(Folder f, int msgno)
				throws MessagingException {
	io.wabw.misc.mail.pop3.POP3Message m = null;
	Constructor cons = store.messageConstructor;
	if (cons != null) {
	    try {
		Object[] o = { this, new Integer(msgno) };
		m = (io.wabw.misc.mail.pop3.POP3Message)cons.newInstance(o);
	    } catch (Exception ex) {
		// ignore
	    }
	}
	if (m == null)
	    m = new io.wabw.misc.mail.pop3.POP3Message(this, msgno);
	return m;
    }
	...
	if (!doneUidl && store.supportsUidl &&
		...
		if (uids[i] == null)
		    continue;
		io.wabw.misc.mail.pop3.POP3Message m = (io.wabw.misc.mail.pop3.POP3Message)getMessage(i + 1);
		m.uid = uids[i];
	    }
	...
	if (fp.contains(FetchProfile.Item.ENVELOPE)) {
		...
		try {
		    io.wabw.misc.mail.pop3.POP3Message msg = (io.wabw.misc.mail.pop3.POP3Message)msgs[i];
		    // fetch headers
		    msg.getHeader("");
		    // fetch message size
		    msg.getSize();
	...
	checkOpen();
	io.wabw.misc.mail.pop3.POP3Message m = (io.wabw.misc.mail.pop3.POP3Message)msg;
	try {
	    if (!store.supportsUidl)
		return null;
	    if (m.uid == io.wabw.misc.mail.pop3.POP3Message.UNKNOWN)
    ...
    */
     ...
     * is closed.
     */
    io.wabw.misc.mail.pop3.Protocol getProtocol() throws MessagingException {
	io.wabw.misc.mail.pop3.Protocol p = port;	// read it before close() can set it to null
	checkOpen();
     ...
     * Used by POP3Message.
     */
    io.wabw.misc.mail.pop3.TempFile getFileCache() {
	return fileCache;
    }
}
