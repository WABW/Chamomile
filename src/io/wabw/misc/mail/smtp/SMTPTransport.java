...
package io.wabw.misc.mail.smtp;
...
public class SMTPTransport extends Transport {
    ...
    private io.wabw.misc.mail.smtp.SMTPOutputStream dataStream;
    ...
    private io.wabw.misc.mail.smtp.SaslAuthenticator saslAuthenticator; // if SASL is being used
	...
	noauthdebug = debug && !io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
	    "mail.debug.auth", false);
	if (urlname != null)
	    name = urlname.getProtocol();
	this.name = name;
	if (!isSSL)
	    isSSL = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
	...
	quitWait = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
				"mail." + name + ".quitwait", true);

	// mail.smtp.reportsuccess causes us to throw an exception on success
	reportSuccess = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
				"mail." + name + ".reportsuccess", false);

	// mail.smtp.starttls.enable enables use of STARTTLS command
	useStartTLS = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
				"mail." + name + ".starttls.enable", false);

	// mail.smtp.starttls.required requires use of STARTTLS command
	requireStartTLS = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
				"mail." + name + ".starttls.required", false);

	// mail.smtp.userset causes us to use RSET instead of NOOP
	// for isConnected
	useRset = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
				"mail." + name + ".userset", false);

	// mail.smtp.noop.strict requires 250 response to indicate success
	noopStrict = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
				"mail." + name + ".noop.strict", true);

	// check if SASL is enabled
	enableSASL = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
	...
	boolean useEhlo =  io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
					"mail." + name + ".ehlo", true);
	// setting mail.smtp.auth to true enables attempts to use AUTH
	boolean useAuth = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
	 ...
	 */
        if (port == -1)
	    port = io.wabw.misc.mail.util.PropUtil.getIntSessionProperty(session,
					"mail." + name + ".port", -1);
        if (port == -1)
	    port = defaultPort;
	...
	while (st.hasMoreTokens()) {
	    ...
	    boolean disabled =
		io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session, dprop, false);
	    if (disabled) {
		if (debug)
	...
	void doAuth(String host, String authzid, String user, String passwd)
	    ...
	    // send username
	    resp = simpleCommand(
		io.wabw.misc.mail.util.BASE64EncoderStream.encode(io.wabw.misc.mail.util.ASCIIUtility.getBytes(user)));
	    if (resp == 334) {
		// send passwd
		resp = simpleCommand(
		    io.wabw.misc.mail.util.BASE64EncoderStream.encode(io.wabw.misc.mail.util.ASCIIUtility.getBytes(passwd)));
	    }
	}
    }
	...
	String getInitialResponse(String host, String authzid, String user,
	    ...
	    OutputStream b64os =
			new io.wabw.misc.mail.util.BASE64EncoderStream(bos, Integer.MAX_VALUE);
	    if (authzid != null)
		b64os.write(io.wabw.misc.mail.util.ASCIIUtility.getBytes(authzid));
	    b64os.write(0);
	    b64os.write(io.wabw.misc.mail.util.ASCIIUtility.getBytes(user));
	    b64os.write(0);
	    b64os.write(io.wabw.misc.mail.util.ASCIIUtility.getBytes(passwd));
	    b64os.flush(); 	// complete the encoding

	    return io.wabw.misc.mail.util.ASCIIUtility.toString(bos.toByteArray());
	}
	...
	private io.wabw.misc.mail.auth.Ntlm ntlm;
	private int flags;
	...
	String getInitialResponse(String host, String authzid, String user,
		String passwd) throws MessagingException, IOException {
	    ntlm = new io.wabw.misc.mail.auth.Ntlm(getNTLMDomain(), getLocalHost(),
				user, passwd, debug ? out : null);

	    flags = io.wabw.misc.mail.util.PropUtil.getIntProperty(
	...
	if (saslAuthenticator == null) {
		...
		Class sac = Class.forName(
                "io.wabw.misc.mail.smtp.SMTPSaslAuthenticator");
		...
		saslAuthenticator = (io.wabw.misc.mail.smtp.SaslAuthenticator)c.newInstance(
					new Object[] {
					this,
					name,
					session.getProperties(),
     ...
     * @see 		javax.mail.event.TransportEvent
     * @exception io.wabw.misc.mail.smtp.SMTPSendFailedException if the send failed because of
	...
	if (message instanceof io.wabw.misc.mail.smtp.SMTPMessage)
	    use8bit = ((io.wabw.misc.mail.smtp.SMTPMessage)message).getAllow8bitMIME();
	if (!use8bit)
	    use8bit = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
	...
	try {
		...
		throw new io.wabw.misc.mail.smtp.SMTPSendFailedException(".", lastReturnCode,
    ...
    private io.wabw.misc.mail.util.LineInputStream lineInputStream;
    private OutputStream        serverOutput;
    private Socket              serverSocket;
    private io.wabw.misc.mail.util.TraceInputStream traceInput;
    private io.wabw.misc.mail.util.TraceOutputStream traceOutput;
	...
	String from = null;
	if (message instanceof io.wabw.misc.mail.smtp.SMTPMessage)
	    from = ((io.wabw.misc.mail.smtp.SMTPMessage)message).getEnvelopeFrom();
	...
	if (supportsExtension("DSN")) {
	    String ret = null;
	    if (message instanceof io.wabw.misc.mail.smtp.SMTPMessage)
		ret = ((io.wabw.misc.mail.smtp.SMTPMessage)message).getDSNRet();
	    if (ret == null)
	...
	if (supportsExtension("AUTH")) {
	    ...
	    if (message instanceof io.wabw.misc.mail.smtp.SMTPMessage)
		submitter = ((io.wabw.misc.mail.smtp.SMTPMessage)message).getSubmitter();
	    if (submitter == null)
	...
	String ext = null;
	if (message instanceof io.wabw.misc.mail.smtp.SMTPMessage)
	    ext = ((io.wabw.misc.mail.smtp.SMTPMessage)message).getMailExtension();
	if (ext == null)
	...
	} catch (io.wabw.misc.mail.smtp.SMTPSendFailedException ex) {
	    int retCode = ex.getReturnCode();
	    switch (retCode) {
	    case 550: case 553: case 503: case 551: case 501:
		// given address is invalid
		try {
		    ex.setNextException(new io.wabw.misc.mail.smtp.SMTPSenderFailedException(
	...
	if (message instanceof io.wabw.misc.mail.smtp.SMTPMessage)
	    sendPartial = ((io.wabw.misc.mail.smtp.SMTPMessage)message).getSendPartial();
	if (!sendPartial)
	    sendPartial = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
	...
	if (supportsExtension("DSN")) {
	    if (message instanceof io.wabw.misc.mail.smtp.SMTPMessage)
		notify = ((io.wabw.misc.mail.smtp.SMTPMessage)message).getDSNNotify();
	    if (notify == null)
	...
	for (int i = 0; i < addresses.length; i++) {
		...
		// create and chain the exception
		sfex = new io.wabw.misc.mail.smtp.SMTPAddressFailedException(ia, cmd, retCode,
							lastServerResponse);
		if (mex == null)
		    mex = sfex;
		else
		...
		if (retCode >= 400 && retCode <= 499) {
		    ...
		    lastReturnCode = _lrc;
		    throw new io.wabw.misc.mail.smtp.SMTPAddressFailedException(ia, cmd, retCode,
								_lsr);
		}
		if (!sendPartial)
		    sendFailed = true;
	...
	dataStream = new io.wabw.misc.mail.smtp.SMTPOutputStream(serverOutput);
	return dataStream;
    }
	...
	try {
	    serverSocket = io.wabw.misc.mail.util.SocketFetcher.startTLS(serverSocket, host,
	...
	try {
	    ...
	    serverSocket = io.wabw.misc.mail.util.SocketFetcher.getSocket(host, port,
	...
	boolean quote = io.wabw.misc.mail.util.PropUtil.getBooleanSessionProperty(session,
					"mail.debug.quote", false);

	traceInput = new io.wabw.misc.mail.util.TraceInputStream(serverSocket.getInputStream(), out);
	traceInput.setTrace(debug);
	traceInput.setQuote(quote);

	traceOutput =
	    new io.wabw.misc.mail.util.TraceOutputStream(serverSocket.getOutputStream(), out);
	...
	lineInputStream = new io.wabw.misc.mail.util.LineInputStream(serverInput);
    }
	...
	if ((ret = readServerResponse()) != expect) {
	    ...
	    lastServerResponse = _lsr;	// restore, for get
	    lastReturnCode = _lrc;
	    throw new io.wabw.misc.mail.smtp.SMTPSendFailedException(cmd, ret, lastServerResponse,
	...
	sendCommand(io.wabw.misc.mail.util.ASCIIUtility.getBytes(cmd));
    }
