...
package io.wabw.misc.mail.imap;
...
public class IMAPFolder extends Folder implements UIDFolder, io.wabw.misc.mail.iap.ResponseHandler {
    ...
    protected volatile io.wabw.misc.mail.imap.protocol.IMAPProtocol protocol; // this folder's protocol object
    protected io.wabw.misc.mail.imap.MessageCache messageCache;// message cache
    ...
    private io.wabw.misc.mail.imap.protocol.Status cachedStatus = null;
     ...
     */
    protected IMAPFolder(String fullName, char separator, io.wabw.misc.mail.imap.IMAPStore store,
				Boolean isNamespace) {
	super(store);
	if (fullName == null)
	...
	this.separator = separator;
        ...
        connectionPoolDebug = ((io.wabw.misc.mail.imap.IMAPStore)store).getConnectionPoolDebug();
     ...
     */
    protected IMAPFolder(io.wabw.misc.mail.imap.protocol.ListInfo li, io.wabw.misc.mail.imap.IMAPStore store) {
	...
	synchronized(messageCacheLock) { // Acquire lock
	    ...
	    } catch (io.wabw.misc.mail.iap.ConnectionException cex) {
	...
	*/
	...
	if ((index = fullName.lastIndexOf(c)) != -1)
	    return ((io.wabw.misc.mail.imap.IMAPStore)store).newIMAPFolder(
			    fullName.substring(0, index), c);
	else
	    return new DefaultFolder((io.wabw.misc.mail.imap.IMAPStore)store);
    }
	...
	io.wabw.misc.mail.imap.protocol.ListInfo[] li = null;
	final String lname;
	...
	li = (io.wabw.misc.mail.imap.protocol.ListInfo[])doCommand(new ProtocolCommand() {
	    public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p) throws io.wabw.misc.mail.iap.ProtocolException {
		return p.list("", lname);
	    }
	});
     ...
     * returned.
     */
    private int findName(io.wabw.misc.mail.imap.protocol.ListInfo[] li, String lname) {
	int i;
	...
	io.wabw.misc.mail.imap.protocol.ListInfo[] li = (io.wabw.misc.mail.imap.protocol.ListInfo[])doCommandIgnoreFailure(
	    new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
	...
	io.wabw.misc.mail.imap.IMAPStore st = (io.wabw.misc.mail.imap.IMAPStore)store;
	...
	try {
	    li = (io.wabw.misc.mail.imap.protocol.ListInfo[])doProtocolCommand(new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
			throws io.wabw.misc.mail.iap.ProtocolException {
		    return p.lsub("", lname);
		}
	...
	Object ret = doCommandIgnoreFailure(new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
			...
			if ((type & HOLDS_FOLDERS) != 0) {
			    ...
			    io.wabw.misc.mail.imap.protocol.ListInfo[] li = p.list("", fullName);
			    if (li != null && !li[0].hasInferiors) {
				// Hmm ..the new folder 
				// doesn't support Inferiors ? Fail
				p.delete(fullName);
				throw new io.wabw.misc.mail.iap.ProtocolException("Unsupported type");
			    }
			}
		    }
		    return Boolean.TRUE;
	...
	if (isNamespace && separator != '\0')
	    ...
	    lname = fullName;
	li = (io.wabw.misc.mail.imap.protocol.ListInfo[])doCommandIgnoreFailure(new ProtocolCommand() {
	    public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p) throws io.wabw.misc.mail.iap.ProtocolException {
		return p.list("", lname);
	    }
	});
	...
	int i = findName(li, lname);
	if (li[i].changeState == io.wabw.misc.mail.imap.protocol.ListInfo.CHANGED)
	    return true;
	else if (li[i].changeState == io.wabw.misc.mail.imap.protocol.ListInfo.UNCHANGED)
	    return false;

	// LIST didn't work. Try the hard way, using STATUS
	try {
	    io.wabw.misc.mail.imap.protocol.Status status = getStatus();
	    if (status.recent > 0)
		return true;
	    else
		return false;
	} catch (io.wabw.misc.mail.iap.BadCommandException bex) {
	...
	char c = getSeparator();
	return ((io.wabw.misc.mail.imap.IMAPStore)store).newIMAPFolder(fullName + c + name, c);
    }
	...
	io.wabw.misc.mail.imap.protocol.MailboxInfo mi = null;
	// Request store for our own protocol connection.
	protocol = ((io.wabw.misc.mail.imap.IMAPStore)store).getProtocol(this);
	...
	synchronized(messageCacheLock) { // Acquire messageCacheLock
	    ...
	    } catch (io.wabw.misc.mail.iap.CommandFailedException cex) {
		/*
		...
		try {
		    protocol.logout();
		} catch (io.wabw.misc.mail.iap.ProtocolException pex2) {
		    // ignore
		} finally {
		...
		if (mode == READ_WRITE && mi.mode == READ_ONLY &&
			((io.wabw.misc.mail.imap.IMAPStore)store).allowReadOnlySelect()) {
		    ;		// all ok, allow it
	    ...
	    messageCache = new io.wabw.misc.mail.imap.MessageCache(this, (io.wabw.misc.mail.imap.IMAPStore)store, total);
    ...
    public synchronized void fetch(Message[] msgs, FetchProfile fp)
			throws MessagingException {
	checkOpened();
	io.wabw.misc.mail.imap.IMAPMessage.fetch(this, msgs, fp);
    }
	...
	synchronized(messageCacheLock) {
	    try {
		io.wabw.misc.mail.imap.protocol.IMAPProtocol p = getProtocol();
		io.wabw.misc.mail.imap.protocol.MessageSet[] ms = io.wabw.misc.mail.imap.Utility.toMessageSet(msgs, null);
		if (ms == null)
	...
	synchronized(messageCacheLock) {
		...
		if (force) {
		    ...
		    if (protocol != null)
			protocol.disconnect();
                } else if (((io.wabw.misc.mail.imap.IMAPStore)store).isConnectionPoolFull()) {
		    ...
		    // before closing, or unselect it if supported.
				...
				if (protocol != null) {
				    io.wabw.misc.mail.imap.protocol.MailboxInfo mi = protocol.examine(fullName);
	...
	if (!opened) {
		...
		// lets try EXAMINE
                io.wabw.misc.mail.imap.protocol.IMAPProtocol p = null;

	        try {
	            p = getStoreProtocol();	// XXX
		    io.wabw.misc.mail.imap.protocol.MailboxInfo minfo = p.examine(fullName);
		    p.close();
		    return minfo.total;
		} catch (io.wabw.misc.mail.iap.ProtocolException pex) {
		    // Give up.
     ...
     */
    private io.wabw.misc.mail.imap.protocol.Status getStatus() throws io.wabw.misc.mail.iap.ProtocolException {
	int statusCacheTimeout = ((io.wabw.misc.mail.imap.IMAPStore)store).getStatusCacheTimeout();
	...
	try {
	    p = getStoreProtocol();	// XXX
	    io.wabw.misc.mail.imap.protocol.Status s = p.status(fullName, null);
	...
	int maxsize = ((io.wabw.misc.mail.imap.IMAPStore)store).getAppendBufferSize();
     ...
     * @since	JavaMail 1.4
     */
    public synchronized io.wabw.misc.mail.imap.AppendUID[] appendUIDMessages(Message[] msgs)
				throws MessagingException {
	checkExists(); // verify that self exists
	...
	io.wabw.misc.mail.imap.AppendUID[] uids = new io.wabw.misc.mail.imap.AppendUID[msgs.length];
	for (int i = 0; i < msgs.length; i++) {
	    final Message m = msgs[i];
	    final MessageLiteral mos;
	    ...
	    final Date dd = d;
	    final Flags f = m.getFlags();
	    io.wabw.misc.mail.imap.AppendUID auid = (io.wabw.misc.mail.imap.AppendUID)doCommand(new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
			throws io.wabw.misc.mail.iap.ProtocolException {
	...
	io.wabw.misc.mail.imap.AppendUID[] uids = appendUIDMessages(msgs);
	for (int i = 0; i < uids.length; i++) {
	    io.wabw.misc.mail.imap.AppendUID auid = uids[i];
	    if (auid != null) {
	...
	io.wabw.misc.mail.imap.IMAPMessage[] rmsgs;
	synchronized(messageCacheLock) {
	    doExpungeNotification = false; // We do this ourselves later
	    try {
		io.wabw.misc.mail.imap.protocol.IMAPProtocol p = getProtocol();
		if (msgs != null)
		    p.uidexpunge(io.wabw.misc.mail.imap.Utility.toUIDSet(msgs));
		else
		    p.expunge();
	    } catch (io.wabw.misc.mail.iap.CommandFailedException cfx) {
		...
		for (int i = 0; i < rmsgs.length; i++) {
		    io.wabw.misc.mail.imap.IMAPMessage m = rmsgs[i];
	...
	try {
		...
		if (matches != null) {
		    matchMsgs = new io.wabw.misc.mail.imap.IMAPMessage[matches.length];
	...
	if (opened) // we already have this information
        ...
        io.wabw.misc.mail.imap.protocol.IMAPProtocol p = null;
        io.wabw.misc.mail.imap.protocol.Status status = null;
	...
	io.wabw.misc.mail.imap.IMAPMessage m = null;
	...
	try {
		...
		if (uidTable != null) {
		    // Check in uidTable
		    m = (io.wabw.misc.mail.imap.IMAPMessage)uidTable.get(l);
		    if (m != null) // found it
			return m;
		} else
		...
		io.wabw.misc.mail.imap.protocol.UID u = getProtocol().fetchSequenceNumber(uid);
	...
	try {
		...
		io.wabw.misc.mail.imap.protocol.UID[] ua = getProtocol().fetchSequenceNumbers(start, end);

		msgs = new Message[ua.length];
		io.wabw.misc.mail.imap.IMAPMessage m;
	...
	try {
		...
		if (unavailUids.length > 0) {
		    ...
		    io.wabw.misc.mail.imap.protocol.UID[] ua = getProtocol().fetchSequenceNumbers(unavailUids);
		    io.wabw.misc.mail.imap.IMAPMessage m;
		    for (int i = 0; i < ua.length; i++) {
			m = getMessageBySeqNumber(ua[i].seqnum);
			m.setUID(ua[i].uid);
	...
	io.wabw.misc.mail.imap.IMAPMessage m = (io.wabw.misc.mail.imap.IMAPMessage)message;
	...
	synchronized(messageCacheLock) { // Acquire Lock
		...
		io.wabw.misc.mail.imap.protocol.UID u = p.fetchUID(m.getSequenceNumber());
     ...
     *					ACL extension
     */
    public io.wabw.misc.mail.imap.ACL[] getACL() throws MessagingException {
	return (io.wabw.misc.mail.imap.ACL[])doOptionalCommand("ACL not supported",
	    new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
			throws io.wabw.misc.mail.iap.ProtocolException {
     ...
     *					ACL extension
     */
    public void addACL(io.wabw.misc.mail.imap.ACL acl) throws MessagingException {
	setACL(acl, '\0');
    }
     ...
     *					ACL extension
     */
    public void addRights(io.wabw.misc.mail.imap.ACL acl) throws MessagingException {
	setACL(acl, '+');
    }
     ...
     *					ACL extension
     */
    public void removeRights(io.wabw.misc.mail.imap.ACL acl) throws MessagingException {
	setACL(acl, '-');
    }
     ...
     *					ACL extension
     */
    public io.wabw.misc.mail.imap.Rights[] listRights(final String name) throws MessagingException {
	return (io.wabw.misc.mail.imap.Rights[])doOptionalCommand("ACL not supported",
	    new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
			throws io.wabw.misc.mail.iap.ProtocolException {
     ...
     *					ACL extension
     */
    public io.wabw.misc.mail.imap.Rights myRights() throws MessagingException {
	return (io.wabw.misc.mail.imap.Rights)doOptionalCommand("ACL not supported",
	    new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
			throws io.wabw.misc.mail.iap.ProtocolException {
    ...
    private void setACL(final io.wabw.misc.mail.imap.ACL acl, final char mod)
				throws MessagingException {
	doOptionalCommand("ACL not supported",
	    new ProtocolCommand() {
		public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol p)
	 ...
	 */
	for (;;) {
	    io.wabw.misc.mail.iap.Response r = protocol.readIdleResponse();
	    try {
		synchronized (messageCacheLock) {
		    try {
	 ...
	 * were idle.
	 */
	int minidle = ((io.wabw.misc.mail.imap.IMAPStore)store).getMinIdleTime();
	if (minidle > 0) {
	    try {
     ...
     */
    void waitIfIdle() throws io.wabw.misc.mail.iap.ProtocolException {
	assert Thread.holdsLock(messageCacheLock);
     ...
     */
    public void handleResponse(io.wabw.misc.mail.iap.Response r) {
	assert Thread.holdsLock(messageCacheLock);
	...
	if (r.isOK() || r.isNO() || r.isBAD() || r.isBYE())
	    ((io.wabw.misc.mail.imap.IMAPStore)store).handleResponseCode(r);
	...
	if (!(r instanceof io.wabw.misc.mail.imap.protocol.IMAPResponse)) {
	...
	if (ir.keyEquals("EXISTS")) { // EXISTS
	    ...
	    assert ir instanceof io.wabw.misc.mail.imap.protocol.FetchResponse : "!ir instanceof FetchResponse";
	    io.wabw.misc.mail.imap.protocol.FetchResponse f = (io.wabw.misc.mail.imap.protocol.FetchResponse)ir;
	    // Get FLAGS response, if present
	    Flags flags = (Flags)f.getItem(Flags.class);

	    if (flags != null) {
		io.wabw.misc.mail.imap.IMAPMessage msg = getMessageBySeqNumber(f.getNumber());
     ...
     * 	messageCacheLock
     */
    void handleResponses(io.wabw.misc.mail.iap.Response[] r) {
     ...
     *     }
     */
    protected synchronized io.wabw.misc.mail.imap.protocol.IMAPProtocol getStoreProtocol()
            throws io.wabw.misc.mail.iap.ProtocolException {
	if (connectionPoolDebug) {
	    out.println("DEBUG: getStoreProtocol() - " + 
		"borrowing a connection");
	}
	return ((io.wabw.misc.mail.imap.IMAPStore)store).getFolderStoreProtocol();
    }

    /**
     * Throw the appropriate 'closed' exception.
     */
    protected synchronized void throwClosedException(io.wabw.misc.mail.iap.ConnectionException cex)
     ...
     */
    protected io.wabw.misc.mail.imap.protocol.IMAPProtocol getProtocol() throws io.wabw.misc.mail.iap.ProtocolException {
	assert Thread.holdsLock(messageCacheLock);
	waitIfIdle();
        return protocol;
    }
	 ...
	 * object.
	 */
	public Object doCommand(io.wabw.misc.mail.imap.protocol.IMAPProtocol protocol) throws io.wabw.misc.mail.iap.ProtocolException;
    }
     ...
     */
    protected synchronized void releaseStoreProtocol(io.wabw.misc.mail.imap.protocol.IMAPProtocol p) {
        if (p != protocol)
            ((io.wabw.misc.mail.imap.IMAPStore)store).releaseFolderStoreProtocol(p);
	else {
    ...
    protected void releaseProtocol(boolean returnToPool) {
        if (protocol != null) {
            ...
            if (returnToPool)
                ((io.wabw.misc.mail.imap.IMAPStore)store).releaseProtocol(this, protocol);
            else {
		protocol.disconnect();	// make sure it's disconnected
                ((io.wabw.misc.mail.imap.IMAPStore)store).releaseProtocol(this, null);
	    }
	    protocol = null;
        }
    }
    ...
    protected void keepConnectionAlive(boolean keepStoreAlive) 
        ...
        if (keepStoreAlive && ((io.wabw.misc.mail.imap.IMAPStore)store).hasSeparateStoreConnection()) {
            io.wabw.misc.mail.imap.protocol.IMAPProtocol p = null;
	    try {
		p = ((io.wabw.misc.mail.imap.IMAPStore)store).getFolderStoreProtocol();
     ...
     *  messageCacheLock
     */
    protected io.wabw.misc.mail.imap.IMAPMessage getMessageBySeqNumber(int seqnum) {
 ...
 */
class MessageLiteral implements io.wabw.misc.mail.iap.Literal {
    private Message msg;
	...
	OutputStream os = new io.wabw.misc.mail.util.CRLFOutputStream(lc);
	msg.writeTo(os);
	os.flush();
	msgSize = lc.getSize();
	buf = lc.getBytes();
	...
	try {
		...
		os = new io.wabw.misc.mail.util.CRLFOutputStream(os);
		msg.writeTo(os);
	    }
	} catch (MessagingException mex) {
