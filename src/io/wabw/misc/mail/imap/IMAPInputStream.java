...
public class IMAPInputStream extends InputStream {
    ...
    private io.wabw.misc.mail.iap.ByteArray readbuf; // reuse for each read
    ...
    private void forceCheckExpunged()
		    throws io.wabw.misc.mail.util.MessageRemovedIOException, io.wabw.misc.mail.util.FolderClosedIOException {
	synchronized (msg.getMessageCacheLock()) {
	    try {
		msg.getProtocol().noop();
	    } catch (io.wabw.misc.mail.iap.ConnectionException cex) {
		throw new io.wabw.misc.mail.util.FolderClosedIOException(msg.getFolder(),
						cex.getMessage());
	    } catch (FolderClosedException fex) {
		throw new io.wabw.misc.mail.util.FolderClosedIOException(fex.getFolder(),
						fex.getMessage());
	...
	if (msg.isExpunged())
	    throw new io.wabw.misc.mail.util.MessageRemovedIOException();
    }
	...
	io.wabw.misc.mail.imap.protocol.BODY b = null;
	if (readbuf == null)
	    readbuf = new io.wabw.misc.mail.iap.ByteArray(blksize + slop);

	io.wabw.misc.mail.iap.ByteArray ba;
	// Acquire MessageCacheLock, to freeze seqnum.
	synchronized (msg.getMessageCacheLock()) {
	    try {
		io.wabw.misc.mail.imap.protocol.IMAPProtocol p = msg.getProtocol();
