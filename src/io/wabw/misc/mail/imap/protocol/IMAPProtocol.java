...
public class IMAPProtocol extends io.wabw.misc.mail.iap.Protocol {
    ...
    private io.wabw.misc.mail.iap.ByteArray ba;		// a buffer for fetchBody
	...
	try {
	    this.name = name;
	    noauthdebug = debug && !io.wabw.misc.mail.util.PropUtil.getBooleanProperty(props,
     ...
     */
    public void capability() throws io.wabw.misc.mail.iap.ProtocolException {
	// Check CAPABILITY
	io.wabw.misc.mail.iap.Response[] r = command("CAPABILITY", null);

	if (!r[r.length-1].isOK())
	    throw new io.wabw.misc.mail.iap.ProtocolException(r[r.length-1].toString());

	capabilities = new HashMap(10);
	authmechs = new ArrayList(5);
	for (int i = 0, len = r.length; i < len; i++) {
	    if (!(r[i] instanceof io.wabw.misc.mail.imap.protocol.IMAPResponse))
		continue;

	    io.wabw.misc.mail.imap.protocol.IMAPResponse ir = (io.wabw.misc.mail.imap.protocol.IMAPResponse)r[i];
     ...
     */
    protected void setCapabilities(io.wabw.misc.mail.iap.Response r) {
	byte b;
     ...
     */
    protected void parseCapabilities(io.wabw.misc.mail.iap.Response r) {
	String s;
	...
	io.wabw.misc.mail.imap.protocol.IMAPResponse ir = (io.wabw.misc.mail.imap.protocol.IMAPResponse)r;
     ...
     */
    public io.wabw.misc.mail.iap.Response readResponse() throws IOException, io.wabw.misc.mail.iap.ProtocolException {
	// assert Thread.holdsLock(this);
	// can't assert because it's called from constructor
	return io.wabw.misc.mail.imap.protocol.IMAPResponse.readResponse(this);
    }
     ...
     */
    public void noop() throws io.wabw.misc.mail.iap.ProtocolException {
	if (debug)
     ...
     */
    public void logout() throws io.wabw.misc.mail.iap.ProtocolException {
	try {
	    io.wabw.misc.mail.iap.Response[] r = command("LOGOUT", null);
     ...
     */
    public void login(String u, String p) throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Argument args = new io.wabw.misc.mail.iap.Argument();
	args.writeString(u);
	args.writeString(p);
	...
	try {
	    ...
	    r = io.wabw.misc.mail.iap.Response.byeResponse(ex);
	    done = true;
	}
	...
	OutputStream b64os = new io.wabw.misc.mail.util.BASE64EncoderStream(bos, Integer.MAX_VALUE);
	boolean first = true;
	...
	while (!done) { // loop till we are done
		...
		r = readResponse();
		    ...
		    b64os.write(io.wabw.misc.mail.util.ASCIIUtility.getBytes(s));
     ...
     * @throws io.wabw.misc.mail.iap.ProtocolException as thrown by {@link io.wabw.misc.mail.iap.Protocol#handleResult}.
	...
	int flags = io.wabw.misc.mail.util.PropUtil.getIntProperty(props,
	    "mail." + name + ".auth.ntlm.flags", 0);
	String domain = props.getProperty(
	    "mail." + name + ".auth.ntlm.domain", "");
	io.wabw.misc.mail.auth.Ntlm ntlm = new io.wabw.misc.mail.auth.Ntlm(domain, getLocalHost(), u, p, debug ? out : null);
	...
	while (!done) { // loop till we are done
		...
		r = readResponse();
		    ...
		    os.write(io.wabw.misc.mail.util.ASCIIUtility.getBytes(s));
    ...
    public void sasllogin(String[] allowed, String realm, String authzid,
				String u, String p) throws io.wabw.misc.mail.iap.ProtocolException {
	if (saslAuthenticator == null) {
	    try {
		Class sac = Class.forName(
                "io.wabw.misc.mail.imap.protocol.IMAPSaslAuthenticator");
     ...
     */
    public void proxyauth(String u) throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Argument args = new io.wabw.misc.mail.iap.Argument();
	args.writeString(u);
     ...
     */
    public void id(String guid) throws io.wabw.misc.mail.iap.ProtocolException {
	/*
     ...
     */
    public void startTLS() throws io.wabw.misc.mail.iap.ProtocolException {
	try {
	    ...
	    io.wabw.misc.mail.iap.Response[] r = { io.wabw.misc.mail.iap.Response.byeResponse(ex) };
	    notifyResponseHandlers(r);
	    disconnect();
	    throw new io.wabw.misc.mail.iap.ProtocolException("STARTTLS failure", ex);
	}
    }
     ...
     */
    public MailboxInfo select(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	...
	io.wabw.misc.mail.iap.Response[] r = command("SELECT", args);
	...
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
     ...
     */
    public MailboxInfo examine(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	// encode the mbox as per RFC2060
	mbox = BASE64MailboxEncoder.encode(mbox);
	...
	io.wabw.misc.mail.iap.Response[] r = command("EXAMINE", args);
     ...
     * @since	JavaMail 1.4.4
     */
    public void unselect() throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("UNSELECT")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("UNSELECT not supported");
	...
	if (!isREV1() && !hasCapability("IMAP4SUNVERSION")) 
	    ...
	    // does support this.
	    throw new io.wabw.misc.mail.iap.BadCommandException("STATUS not supported");
	...
	io.wabw.misc.mail.iap.Argument itemArgs = new io.wabw.misc.mail.iap.Argument();
	if (items == null)
	...
	io.wabw.misc.mail.iap.Response[] r = command("STATUS", args);
     ...
     */
    public void create(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	// encode the mbox as per RFC2060
	mbox = BASE64MailboxEncoder.encode(mbox);
     ...
     */
    public void delete(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	// encode the mbox as per RFC2060
	mbox = BASE64MailboxEncoder.encode(mbox);
     ...
     */
    public void rename(String o, String n) throws io.wabw.misc.mail.iap.ProtocolException {
	// encode the mbox as per RFC2060
     ...
     */
    public void subscribe(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Argument args = new io.wabw.misc.mail.iap.Argument();
	// encode the mbox as per RFC2060
	mbox = BASE64MailboxEncoder.encode(mbox);
	args.writeString(mbox);
     ...
     */
    public void unsubscribe(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Argument args = new io.wabw.misc.mail.iap.Argument();
	// encode the mbox as per RFC2060
	mbox = BASE64MailboxEncoder.encode(mbox);
	args.writeString(mbox);
    ...
    public void append(String mbox, Flags f, Date d,
			io.wabw.misc.mail.iap.Literal data) throws io.wabw.misc.mail.iap.ProtocolException {
     ...
     * @see "RFC2060, section 6.3.11"
     */
    public io.wabw.misc.mail.imap.AppendUID appenduid(String mbox, Flags f, Date d,
                                                      io.wabw.misc.mail.iap.Literal data) throws io.wabw.misc.mail.iap.ProtocolException {
	return appenduid(mbox, f, d, data, true);
    }

    public io.wabw.misc.mail.imap.AppendUID appenduid(String mbox, Flags f, Date d,
                                                      io.wabw.misc.mail.iap.Literal data, boolean uid) throws io.wabw.misc.mail.iap.ProtocolException {
	// encode the mbox as per RFC2060
	mbox = BASE64MailboxEncoder.encode(mbox);
	...
	io.wabw.misc.mail.iap.Response[] r = command("APPEND", args);
     ...
     */
    private io.wabw.misc.mail.imap.AppendUID getAppendUID(io.wabw.misc.mail.iap.Response r) {
	if (!r.isOK())
	    return null;
	byte b;
	while ((b = r.readByte()) > 0 && b != (byte)'[')
	...
	long uid = r.readLong();
	return new io.wabw.misc.mail.imap.AppendUID(uidvalidity, uid);
    }
     ...
     */
    public void check() throws io.wabw.misc.mail.iap.ProtocolException {
     ...
     */
    public void close() throws io.wabw.misc.mail.iap.ProtocolException {
     ...
     */
    public void expunge() throws io.wabw.misc.mail.iap.ProtocolException {
     ...
     */
    public void uidexpunge(UIDSet[] set) throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("UIDPLUS")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("UID EXPUNGE not supported");
    ...
    public BODYSTRUCTURE fetchBodyStructure(int msgno)
			throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Response[] r = fetch(msgno, "BODYSTRUCTURE");
	notifyResponseHandlers(r);
    ...
    protected BODY fetchBody(int msgno, String section, boolean peek)
			throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Response[] r;
     ...
     */
    public BODY peekBody(int msgno, String section, int start, int size,
				io.wabw.misc.mail.iap.ByteArray ba) throws io.wabw.misc.mail.iap.ProtocolException {
    ...
    protected BODY fetchBody(int msgno, String section, int start, int size,
			boolean peek, io.wabw.misc.mail.iap.ByteArray ba) throws io.wabw.misc.mail.iap.ProtocolException {
	this.ba = ba;	// save for later use by getResponseBuffer
	io.wabw.misc.mail.iap.Response[] r = fetch(
	...
	io.wabw.misc.mail.iap.ByteArray ret = ba;
	ba = null;
	return ret;
    }
    ...
    public RFC822DATA fetchRFC822(int msgno, String what)
			throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Response[] r = fetch(msgno,
     ...
     */
    public Flags fetchFlags(int msgno) throws io.wabw.misc.mail.iap.ProtocolException {
	Flags flags = null;
	io.wabw.misc.mail.iap.Response[] r = fetch(msgno, "FLAGS");
     ...
     */
    public UID fetchUID(int msgno) throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Response[] r = fetch(msgno, "UID");
     ...
     */
    public UID fetchSequenceNumber(long uid) throws io.wabw.misc.mail.iap.ProtocolException {
	UID u = null;
	io.wabw.misc.mail.iap.Response[] r = fetch(String.valueOf(uid), "UID", true);
    ...
    public UID[] fetchSequenceNumbers(long start, long end)
			throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Response[] r = fetch(String.valueOf(start) + ":" +
     ...
     * If no UIDs in the given range are found, an empty array is returned.
     */
    public UID[] fetchSequenceNumbers(long[] uids) throws io.wabw.misc.mail.iap.ProtocolException {
	...
	io.wabw.misc.mail.iap.Response[] r = fetch(sb.toString(), "UID", true);
    ...
    public io.wabw.misc.mail.iap.Response[] fetch(MessageSet[] msgsets, String what)
			throws io.wabw.misc.mail.iap.ProtocolException {
	return fetch(MessageSet.toString(msgsets), what, false);
    }

    public io.wabw.misc.mail.iap.Response[] fetch(int start, int end, String what)
			throws io.wabw.misc.mail.iap.ProtocolException {
	return fetch(String.valueOf(start) + ":" + String.valueOf(end), 
		     what, false);
    }

    public io.wabw.misc.mail.iap.Response[] fetch(int msg, String what)
			throws io.wabw.misc.mail.iap.ProtocolException {
	return fetch(String.valueOf(msg), what, false);
    }

    private io.wabw.misc.mail.iap.Response[] fetch(String msgSequence, String what, boolean uid)
			throws io.wabw.misc.mail.iap.ProtocolException {
	if (uid)
    ...
    public int[] search(MessageSet[] msgsets, SearchTerm term)
			throws io.wabw.misc.mail.iap.ProtocolException, SearchException {
	...
	for (int i = 0; i < searchCharsets.length; i++) {
	    ...
	    } catch (io.wabw.misc.mail.iap.CommandFailedException cfx) {
		/*
    ...
    private int[] issueSearch(String msgSequence, SearchTerm term,
      			      String charset) 
	     throws io.wabw.misc.mail.iap.ProtocolException, SearchException, IOException {

	// Generate a search-sequence with the given charset
	io.wabw.misc.mail.iap.Argument args = SearchSequence.generateSequence(term,
			  charset == null ? null : 
     ...
     * @since	JavaMail 1.4.4
     */
    public int[] sort(io.wabw.misc.mail.imap.SortTerm[] term, SearchTerm sterm)
			throws io.wabw.misc.mail.iap.ProtocolException, SearchException {
	if (!hasCapability("SORT*")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("SORT not supported");

	if (term == null || term.length == 0)
	    throw new io.wabw.misc.mail.iap.BadCommandException("Must have at least one sort term");

	io.wabw.misc.mail.iap.Argument args = new io.wabw.misc.mail.iap.Argument();
	io.wabw.misc.mail.iap.Argument sargs = new io.wabw.misc.mail.iap.Argument();
	...
	io.wabw.misc.mail.iap.Response[] r = command("SORT", args);
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
	int[] matches = null;
     ...
     * @see "RFC2342"
     */
    public io.wabw.misc.mail.imap.protocol.Namespaces namespace() throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("NAMESPACE")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("NAMESPACE not supported");

	io.wabw.misc.mail.iap.Response[] r = command("NAMESPACE", null);

	io.wabw.misc.mail.imap.protocol.Namespaces namespace = null;
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
	...
	if (response.isOK()) { // command succesful 
		...
		if (ir.keyEquals("NAMESPACE")) {
		    if (namespace == null)
			namespace = new io.wabw.misc.mail.imap.protocol.Namespaces(ir);
		    r[i] = null;
		}
	    }
	}
     ...
     * @see "RFC2087"
     */
    public Quota[] getQuotaRoot(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("QUOTA")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("GETQUOTAROOT not supported");
	...
	io.wabw.misc.mail.iap.Response[] r = command("GETQUOTAROOT", args);
     ...
     * @see "RFC2087"
     */
    public Quota[] getQuota(String root) throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("QUOTA")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("QUOTA not supported");

	io.wabw.misc.mail.iap.Argument args = new io.wabw.misc.mail.iap.Argument();
	args.writeString(root);

	io.wabw.misc.mail.iap.Response[] r = command("GETQUOTA", args);
     ...
     * @see "RFC2087"
     */
    public void setQuota(Quota quota) throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("QUOTA")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("QUOTA not supported");

	io.wabw.misc.mail.iap.Argument args = new io.wabw.misc.mail.iap.Argument();
	args.writeString(quota.quotaRoot);
	io.wabw.misc.mail.iap.Argument qargs = new io.wabw.misc.mail.iap.Argument();
	...
	io.wabw.misc.mail.iap.Response[] r = command("SETQUOTA", args);
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
	...
	*/
     ...
     */
    private Quota parseQuota(io.wabw.misc.mail.iap.Response r) throws io.wabw.misc.mail.iap.ParsingException {
	...
	if (r.readByte() != '(')
	    throw new io.wabw.misc.mail.iap.ParsingException("parse error in QUOTA");
     ...
     * @see "RFC2086"
     */
    public void setACL(String mbox, char modifier, io.wabw.misc.mail.imap.ACL acl)
				throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("ACL")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("ACL not supported");
	...
	io.wabw.misc.mail.iap.Response[] r = command("SETACL", args);
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
     ...
     * @see "RFC2086"
     */
    public void deleteACL(String mbox, String user) throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("ACL")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("ACL not supported");
	...
	io.wabw.misc.mail.iap.Response[] r = command("DELETEACL", args);
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
     ...
     * @see "RFC2086"
     */
    public io.wabw.misc.mail.imap.ACL[] getACL(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("ACL")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("ACL not supported");
	...
	io.wabw.misc.mail.iap.Response[] r = command("GETACL", args);
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
	...
	if (response.isOK()) { // command succesful 
		...
		if (ir.keyEquals("ACL")) {
			...
			if (rights == null)
			    break;
			io.wabw.misc.mail.imap.ACL acl = new io.wabw.misc.mail.imap.ACL(name, new io.wabw.misc.mail.imap.Rights(rights));
			v.addElement(acl);
		    }
		    r[i] = null;
		}
	...
	notifyResponseHandlers(r);
	handleResult(response);
	io.wabw.misc.mail.imap.ACL[] aa = new io.wabw.misc.mail.imap.ACL[v.size()];
	v.copyInto(aa);
	return aa;
    }
     ...
     * @see "RFC2086"
     */
    public io.wabw.misc.mail.imap.Rights[] listRights(String mbox, String user)
				throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("ACL")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("ACL not supported");
	...
	io.wabw.misc.mail.iap.Response[] r = command("LISTRIGHTS", args);
	io.wabw.misc.mail.iap.Response response = r[r.length-1];
	...
	if (response.isOK()) { // command succesful 
		...
		if (ir.keyEquals("LISTRIGHTS")) {
			...
			v.addElement(new io.wabw.misc.mail.imap.Rights(rights));
		    r[i] = null;
		}
	    }
	}
	...
	notifyResponseHandlers(r);
	handleResult(response);
	io.wabw.misc.mail.imap.Rights[] ra = new io.wabw.misc.mail.imap.Rights[v.size()];
	v.copyInto(ra);
	return ra;
    }
     ...
     * @see "RFC2086"
     */
    public io.wabw.misc.mail.imap.Rights myRights(String mbox) throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("ACL")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("ACL not supported");
	...
	io.wabw.misc.mail.iap.Response[] r = command("MYRIGHTS", args);
	io.wabw.misc.mail.iap.Response response = r[r.length-1];

	// Grab MYRIGHTS response
	io.wabw.misc.mail.imap.Rights rights = null;
	if (response.isOK()) { // command succesful 
	    for (int i = 0, len = r.length; i < len; i++) {
		if (!(r[i] instanceof io.wabw.misc.mail.imap.protocol.IMAPResponse))
		    continue;
		...
		if (ir.keyEquals("MYRIGHTS")) {
		    ...
		    if (rights == null)
			rights = new io.wabw.misc.mail.imap.Rights(rs);
		    r[i] = null;
		}
	    }
	}
     ...
     * @since	JavaMail 1.4.1
     */
    public synchronized void idleStart() throws io.wabw.misc.mail.iap.ProtocolException {
	if (!hasCapability("IDLE")) 
	    throw new io.wabw.misc.mail.iap.BadCommandException("IDLE not supported");
     ...
     * @since	JavaMail 1.4.1
     */
    public synchronized io.wabw.misc.mail.iap.Response readIdleResponse() {
	if (idleTag == null)
	...
	while (r == null) {
		...
		if (iioex.bytesTransferred == 0)
		    ...
		    // convert this into a BYE response
		    r = io.wabw.misc.mail.iap.Response.byeResponse(iioex);
	    } catch (IOException ioex) {
		// convert this into a BYE response
		r = io.wabw.misc.mail.iap.Response.byeResponse(ioex);
	    } catch (io.wabw.misc.mail.iap.ProtocolException pex) {
		// convert this into a BYE response
		r = io.wabw.misc.mail.iap.Response.byeResponse(pex);
	    }
	}
	return r;
    }
     ...
     * @since	JavaMail 1.4.1
     */
    public boolean processIdleResponse(io.wabw.misc.mail.iap.Response r) throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.iap.Response[] responses = new io.wabw.misc.mail.iap.Response[1];
	responses[0] = r;
     ...
     * @since	JavaMail 1.4.1
     */
    public void idleAbort() throws io.wabw.misc.mail.iap.ProtocolException {
