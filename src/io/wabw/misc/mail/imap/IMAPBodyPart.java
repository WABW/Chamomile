...
public class IMAPBodyPart extends MimeBodyPart implements io.wabw.misc.mail.util.ReadableMime {
    private IMAPMessage message;
    private io.wabw.misc.mail.imap.protocol.BODYSTRUCTURE bs;
	...
	io.wabw.misc.mail.util.PropUtil.getBooleanSystemProperty("mail.mime.decodefilename", false);

    protected IMAPBodyPart(io.wabw.misc.mail.imap.protocol.BODYSTRUCTURE bs, String sid, IMAPMessage message) {
	super();
	this.bs = bs;
	this.sectionId = sid;
	...
	boolean pk = message.getPeek();	// acquire outside of message cache lock
	    ...
	    try {
		io.wabw.misc.mail.imap.protocol.IMAPProtocol p = message.getProtocol();

		// Check whether this message is expunged
		message.checkExpunged();

		if (p.isREV1() && (message.getFetchBlockSize() != -1))
		    return new io.wabw.misc.mail.imap.IMAPInputStream(message, sectionId, bs.size, pk);
	...
	synchronized(message.getMessageCacheLock()) {
		...
		if (p.isREV1()) {
		    int seqnum = message.getSequenceNumber();
		    io.wabw.misc.mail.imap.protocol.BODY b = p.peekBody(seqnum, sectionId + ".MIME");
		    ...
		    io.wabw.misc.mail.util.SharedByteArrayOutputStream bos =
			new io.wabw.misc.mail.util.SharedByteArrayOutputStream(0);
		    io.wabw.misc.mail.util.LineOutputStream los = new io.wabw.misc.mail.util.LineOutputStream(bos);
	...
	if (dh == null) {
	   if (bs.isMulti())
		...
		dh = new DataHandler(
			new io.wabw.misc.mail.imap.IMAPNestedMessage(message,
					      bs.bodies[0],
					      bs.envelope,
					      sectionId),
			type
