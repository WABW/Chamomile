...
import io.wabw.misc.mail.util.SocketFetcher;
...
class Protocol {
	...
	if (!r.ok)
	    return false;
	io.wabw.misc.mail.util.LineInputStream lis = new io.wabw.misc.mail.util.LineInputStream(r.bytes);
	String line = null;
	...
	io.wabw.misc.mail.util.SharedByteArrayOutputStream buf = new io.wabw.misc.mail.util.SharedByteArrayOutputStream(size);
	int b, lastb = '\n';
	try {
	    while ((b = input.read()) >= 0) {
		if (lastb == '\n' && b == '.') {
