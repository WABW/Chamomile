...
import io.wabw.misc.mail.util.PropUtil;
...
public class IMAPStore extends Store 
	     implements QuotaAwareStore, io.wabw.misc.mail.iap.ResponseHandler {
    ...
    private io.wabw.misc.mail.imap.protocol.Namespaces namespaces;
	...
	private io.wabw.misc.mail.imap.protocol.IMAPProtocol idleProtocol;	// protocol object when IDLE
     ...
     */
    private io.wabw.misc.mail.iap.ResponseHandler nonStoreResponseHandler = new io.wabw.misc.mail.iap.ResponseHandler() {
	...
	if (s != null) {
		...
		Class[] c2 = { io.wabw.misc.mail.imap.protocol.ListInfo.class, IMAPStore.class };
    ...
    protocolConnect(String host, int pport, String user, String password)
        ...
        io.wabw.misc.mail.imap.protocol.IMAPProtocol protocol = null;
	...
	try {
		...
		if (debug)
		    out.println("DEBUG: trying to connect to host \"" + host +
                ...
                protocol = new io.wabw.misc.mail.imap.protocol.IMAPProtocol(name, host, port,
		    			    session.getDebug(),
		    			    session.getDebugOut(),
	...
	} catch (io.wabw.misc.mail.iap.ProtocolException pex) { // any other exception
    ...
    private void login(io.wabw.misc.mail.imap.protocol.IMAPProtocol p, String u, String pw)
	...
	if (enableStartTLS || requireStartTLS) {
		...
		throw new io.wabw.misc.mail.iap.ProtocolException(
	...
	if (p.isAuthenticated())
	    ...
	    throw new io.wabw.misc.mail.iap.ProtocolException("No login methods supported!");
     ...
     */
    protected void preLogin(io.wabw.misc.mail.imap.protocol.IMAPProtocol p) throws io.wabw.misc.mail.iap.ProtocolException {
    }
     ...
     * open folders.
     */
    io.wabw.misc.mail.imap.protocol.IMAPProtocol getProtocol(IMAPFolder folder)
		throws MessagingException {
	...
	while (p == null) {
        ...
        synchronized (pool) {
			...
			refreshPassword();
                    ...
                    p = new io.wabw.misc.mail.imap.protocol.IMAPProtocol(name, host, port,
                                         session.getDebug(),
                                         session.getDebugOut(),
                                         session.getProperties(),
					 isSSL
                ...
                p = (io.wabw.misc.mail.imap.protocol.IMAPProtocol)pool.authenticatedConnections.lastElement();
     ...
     *     }
     */
    private io.wabw.misc.mail.imap.protocol.IMAPProtocol getStoreProtocol() throws io.wabw.misc.mail.iap.ProtocolException {
        io.wabw.misc.mail.imap.protocol.IMAPProtocol p = null;
	...
	while (p == null) {
	    ...
	    waitIfIdle();
			...
			refreshPassword();
                ...
                if (p == null)
                    throw new io.wabw.misc.mail.iap.ConnectionException(
	        ...
	        p.addResponseHandler(this);
                ...
                if (pool.debug)
                    out.println("DEBUG: getStoreProtocol() - " +
                        ...
                        pool.authenticatedConnections.size());
                p = (io.wabw.misc.mail.imap.protocol.IMAPProtocol)pool.authenticatedConnections.firstElement();
            }
     ...
     */
    io.wabw.misc.mail.imap.protocol.IMAPProtocol getFolderStoreProtocol() throws io.wabw.misc.mail.iap.ProtocolException {
	io.wabw.misc.mail.imap.protocol.IMAPProtocol p = getStoreProtocol();
	p.removeResponseHandler(this);
	p.addResponseHandler(nonStoreResponseHandler);
	return p;
    }
     ...
     */
    void releaseProtocol(IMAPFolder folder, io.wabw.misc.mail.imap.protocol.IMAPProtocol protocol) {
     ...
     */
    private void releaseStoreProtocol(io.wabw.misc.mail.imap.protocol.IMAPProtocol protocol) {
     ...
     */
    void releaseFolderStoreProtocol(io.wabw.misc.mail.imap.protocol.IMAPProtocol protocol) {
	if (protocol == null)
    ...
    private void emptyConnectionPool(boolean force) {
        ...
        synchronized (pool) {
		    ...
		    index >= 0; --index) {
                try {
		    io.wabw.misc.mail.imap.protocol.IMAPProtocol p = (io.wabw.misc.mail.imap.protocol.IMAPProtocol)
    ...
    private void timeoutConnections() {
        ...
        synchronized (pool) {
            ...
            if (System.currentTimeMillis() - pool.lastTimePruned > 
                ...
                io.wabw.misc.mail.imap.protocol.IMAPProtocol p;
                ...
                for (int index = pool.authenticatedConnections.size() - 1; 
                     index > 0; index--) {
                    p = (io.wabw.misc.mail.imap.protocol.IMAPProtocol)pool.authenticatedConnections.
                        elementAt(index);
                    if (pool.debug) {
     ...
     * use it.  Otherwise, call the constructor.
     */
    protected IMAPFolder newIMAPFolder(io.wabw.misc.mail.imap.protocol.ListInfo li) {
	IMAPFolder f = null;
	...
	io.wabw.misc.mail.imap.protocol.Namespaces ns = getNamespaces();
    ...
    private synchronized io.wabw.misc.mail.imap.protocol.Namespaces getNamespaces() throws MessagingException {
	checkConnected();
    ...
    private Folder[] namespaceToFolders(io.wabw.misc.mail.imap.protocol.Namespaces.Namespace[] ns,
                                        String user) {
	...
	try {
	     ...
	     */
	    for (;;) {
		io.wabw.misc.mail.iap.Response r = p.readIdleResponse();
		synchronized (pool) {
     ...
     */
    private void waitIfIdle() throws io.wabw.misc.mail.iap.ProtocolException {
	assert Thread.holdsLock(pool);
     ...
     */
    void handleResponseCode(io.wabw.misc.mail.iap.Response r) {
