...
public class IMAPSaslAuthenticator implements SaslAuthenticator {
    ...
    private io.wabw.misc.mail.imap.protocol.IMAPProtocol pr;
    private String name;
    ...
    public IMAPSaslAuthenticator(io.wabw.misc.mail.imap.protocol.IMAPProtocol pr, String name, Properties props,
    ...
    public boolean authenticate(String[] mechs, final String realm,
				...
				final String p) throws io.wabw.misc.mail.iap.ProtocolException {
	...
	boolean isXGWTRUSTEDAPP =
	    ...
	    io.wabw.misc.mail.util.PropUtil.getBooleanProperty(props,
	...
	while (!done) { // loop till we are done
		...
		r = pr.readResponse();
			...
			if (ba.length > 0)
			    ba = io.wabw.misc.mail.util.BASE64DecoderStream.decode(ba);
			if (debug)
			    out.println("IMAP SASL DEBUG: challenge: " +
				io.wabw.misc.mail.util.ASCIIUtility.toString(ba, 0, ba.length) + " :");
			...
			if (debug)
				...
				io.wabw.misc.mail.util.ASCIIUtility.toString(ba, 0, ba.length) + " :");
			ba = io.wabw.misc.mail.util.BASE64EncoderStream.encode(ba);
			if (isXGWTRUSTEDAPP)
			    bos.write(io.wabw.misc.mail.util.ASCIIUtility.getBytes("XGWTRUSTEDAPP "));
			bos.write(ba);
