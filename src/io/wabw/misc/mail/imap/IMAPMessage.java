...
public class IMAPMessage extends MimeMessage implements io.wabw.misc.mail.util.ReadableMime {
    protected io.wabw.misc.mail.imap.protocol.BODYSTRUCTURE bs;		// BODYSTRUCTURE
    protected io.wabw.misc.mail.imap.protocol.ENVELOPE envelope;	// ENVELOPE
     ...
     */
    protected io.wabw.misc.mail.imap.protocol.IMAPProtocol getProtocol()
			    throws io.wabw.misc.mail.iap.ProtocolException, FolderClosedException {
	((IMAPFolder)folder).waitIfIdle();
	io.wabw.misc.mail.imap.protocol.IMAPProtocol p = ((IMAPFolder)folder).protocol;
	if (p == null)
	...
	boolean pk = getPeek();	// get before acquiring message cache lock
		...
		if (p.isREV1()) {
		    io.wabw.misc.mail.imap.protocol.BODY b;
		    if (pk)
		    ...
		    io.wabw.misc.mail.imap.protocol.RFC822DATA rd = p.fetchRFC822(getSequenceNumber(), "TEXT");
		    if (rd != null)
	...
	boolean pk = getPeek();	// get before acquiring message cache lock
		...
		if (p.isREV1()) {
		    ...
		    io.wabw.misc.mail.imap.protocol.RFC822DATA rd = p.fetchRFC822(getSequenceNumber(), null);
		    if (rd != null)
			is = rd.getByteArrayInputStream();
		}
	    } catch (io.wabw.misc.mail.iap.ConnectionException cex) {
	...
	InputStream is = null;
		...
		if (p.isREV1()) {
		    io.wabw.misc.mail.imap.protocol.BODY b = p.peekBody(getSequenceNumber(),
				toSection("HEADER.FIELDS (" + name + ")")
			     );
		    if (b != null)
			is = b.getByteArrayInputStream();
		} else {
		    io.wabw.misc.mail.imap.protocol.RFC822DATA rd = p.fetchRFC822(getSequenceNumber(),
					"HEADER.LINES (" + name + ")");
		    if (rd != null)
			is = rd.getByteArrayInputStream();
		}
	 ...
	 * breaks out.
	 */
	class FetchProfileCondition implements io.wabw.misc.mail.imap.Utility.Condition {
	...
	io.wabw.misc.mail.imap.Utility.Condition condition = new FetchProfileCondition(fp);
	    ...
	    io.wabw.misc.mail.imap.protocol.MessageSet[] msgsets = io.wabw.misc.mail.imap.Utility.toMessageSet(msgs, condition);
		...
		if (r[i] == null)
		    continue;
		if (!(r[i] instanceof io.wabw.misc.mail.imap.protocol.FetchResponse)) {
		    v.addElement(r[i]); // Unsolicited Non-FETCH response
		    continue;
		}

		// Got a FetchResponse.
		io.wabw.misc.mail.imap.protocol.FetchResponse f = (io.wabw.misc.mail.imap.protocol.FetchResponse)r[i];
		...
		for (int j = 0; j < count; j++) {
		    io.wabw.misc.mail.imap.protocol.Item item = f.getItem(j);
		    ...
		    else if (item instanceof io.wabw.misc.mail.imap.protocol.ENVELOPE)
			msg.envelope = (io.wabw.misc.mail.imap.protocol.ENVELOPE)item;
		    else if (item instanceof io.wabw.misc.mail.imap.protocol.INTERNALDATE)
			msg.receivedDate = ((io.wabw.misc.mail.imap.protocol.INTERNALDATE)item).getDate();
		    else if (item instanceof io.wabw.misc.mail.imap.protocol.RFC822SIZE)
			msg.size = ((io.wabw.misc.mail.imap.protocol.RFC822SIZE)item).size;

		    // Check for the BODYSTRUCTURE item
		    else if (item instanceof io.wabw.misc.mail.imap.protocol.BODYSTRUCTURE)
			msg.bs = (io.wabw.misc.mail.imap.protocol.BODYSTRUCTURE)item;
		    // Check for the UID item
		    else if (item instanceof io.wabw.misc.mail.imap.protocol.UID) {
			io.wabw.misc.mail.imap.protocol.UID u = (io.wabw.misc.mail.imap.protocol.UID)item;
		    ...
		    else if (item instanceof io.wabw.misc.mail.imap.protocol.RFC822DATA ||
			     item instanceof io.wabw.misc.mail.imap.protocol.BODY) {
			InputStream headerStream;
			if (item instanceof io.wabw.misc.mail.imap.protocol.RFC822DATA) // IMAP4
			    headerStream = 
				((io.wabw.misc.mail.imap.protocol.RFC822DATA)item).getByteArrayInputStream();
			else	// IMAP4rev1
			    headerStream = 
				((io.wabw.misc.mail.imap.protocol.BODY)item).getByteArrayInputStream();
	    ...
	    int size = v.size();
	    if (size != 0) {
		io.wabw.misc.mail.iap.Response[] responses = new io.wabw.misc.mail.iap.Response[size];
	...
	synchronized(getMessageCacheLock()) {
		...
		for (int i = 0; i < r.length; i++) {
		    ...
		    if (r[i] == null ||
			!(r[i] instanceof io.wabw.misc.mail.imap.protocol.FetchResponse) ||
			((io.wabw.misc.mail.imap.protocol.FetchResponse)r[i]).getNumber() != seqnum)
			continue;
			...
			if (item instanceof io.wabw.misc.mail.imap.protocol.ENVELOPE)
			    envelope = (io.wabw.misc.mail.imap.protocol.ENVELOPE)item;
			else if (item instanceof io.wabw.misc.mail.imap.protocol.INTERNALDATE)
			    receivedDate = ((io.wabw.misc.mail.imap.protocol.INTERNALDATE)item).getDate();
			else if (item instanceof io.wabw.misc.mail.imap.protocol.RFC822SIZE)
			    size = ((io.wabw.misc.mail.imap.protocol.RFC822SIZE)item).size;
		    }
		}
    ...
    private static String craftHeaderCmd(io.wabw.misc.mail.imap.protocol.IMAPProtocol p, String[] hdrs) {
	StringBuffer sb;
    ...
    private io.wabw.misc.mail.imap.protocol.ENVELOPE _getEnvelope() {
	return envelope;
    }

    private io.wabw.misc.mail.imap.protocol.BODYSTRUCTURE _getBodyStructure() {
	return bs;
    }
