...
public class IMAPResponse extends io.wabw.misc.mail.iap.Response {
    private String key;
    private int number;

    public IMAPResponse(io.wabw.misc.mail.iap.Protocol c) throws IOException, io.wabw.misc.mail.iap.ProtocolException {
	super(c);
	...
	super((io.wabw.misc.mail.iap.Response)r);
	key = r.key;
	number = r.number;
    }
	...
	for (start = index; buffer[index] != ')'; index++) {
		...
		v.addElement(io.wabw.misc.mail.util.ASCIIUtility.toString(buffer, start, index));
    ...
    public static IMAPResponse readResponse(io.wabw.misc.mail.iap.Protocol p)
